...
const automationMode = ref<AutomationMode>('manual')
const activeScene = ref<HomeScenario>('normal')
const loading = ref(false)
const refreshing = ref(false)
const busyDeviceId = ref<string | null>(null)
const busyAutomationMode = ref(false)
const busyScene = ref<HomeScenario | null>(null)
...
const metrics = [
  ...
  { kind: 'smoke', label: '烟雾', color: '#8b5cf6', unit: 'raw', dangerLine: 800 },
  { kind: 'flame', label: '火焰', color: '#ef4444', unit: '状态', dangerLine: 1 },
  { kind: 'presence', label: '有人', color: '#22c55e', unit: '状态', dangerLine: 1 }
] as const

const sceneDeviceTypes: Record<HomeScenario, string[]> = {
  normal: ['light', 'fan', 'water_pump', 'buzzer'],
  sleep: ['light', 'fan', 'buzzer'],
  away: ['water_pump', 'buzzer']
}

const visibleDevices = computed(() =>
  devices.value.filter((device) => sceneDeviceTypes[activeScene.value].includes(device.device_type))
)
...
async function loadHome(showSpinner = false) {
  ...
  try {
    const [deviceList, snapshot, historyData, alertList, automationModeData, sceneData, lockData] = await Promise.all([
      homeApi.listDevices(),
      homeApi.getEnvironment(room),
      homeApi.getEnvironmentHistory(room, 10),
      homeApi.getAlerts(room, true),
      homeApi.getAutomationMode(),
      homeApi.getScenario(),
    ...
    automationMode.value = automationModeData.automation_mode
    activeScene.value = sceneData.scene
...
async function switchAutomationMode(nextMode: AutomationMode) {
  if (activeScene.value !== 'normal') {
    ElMessage.warning('睡眠和离家场景不允许手动切换主控制模式，请先切换到正常场景')
    return
  }
  if (busyAutomationMode.value || automationMode.value === nextMode) return
  busyAutomationMode.value = true
  try {
    const result = await homeApi.setAutomationMode(nextMode)
    if (!result.accepted) ElMessage.warning('设备主控制模式切换未完成')
    else {
      automationMode.value = result.automation_mode
      ElMessage.success(nextMode === 'automatic' ? '自动联动已启用' : '手动控制已启用')
    }
  } catch (error) {
    console.error('Failed to switch automation mode', error)
    ElMessage.error('手动/自动模式切换失败')
  } finally {
    busyAutomationMode.value = false
  }
}

async function runScene(scene: HomeScenario) {
  if (busyScene.value) return
  busyScene.value = scene
  try {
    const result = await homeApi.runScenario(scene)
    if (!result.accepted) ElMessage.warning(result.message || '场景未完全执行')
    else ElMessage.success(`${{ normal: '正常', sleep: '睡眠', away: '离家' }[scene]}场景已生效`)
    await loadHome()
  } catch (error) {
    console.error('Failed to run home scene', error)
    ElMessage.error('场景执行失败')
  } finally {
    busyScene.value = null
  }
}
...
async function toggleDevice(device: HomeDevice) {
  ...
  try {
    const result = device.device_type === 'water_pump'
      ? await homeApi.setPumpState(nextState)
      : device.device_type === 'buzzer'
        ? await homeApi.setBuzzerState(nextState)
        : await homeApi.setDeviceState(device.device_id, nextState)
    if (!result.accepted) {
    ...
    <div class="mx-auto max-w-7xl space-y-6">
      <header class="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <span class="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">当前：{{ automationMode === 'manual' ? '手动' : '自动' }}</span>
          </div>
      ...
      <section class="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div class="flex items-center justify-between"><div class="flex items-center gap-2"><Power :size="18" class="text-indigo-600" /><h2 class="font-semibold">设备主控制模式</h2></div><span class="text-xs text-slate-400">{{ room }}</span></div>
          <div class="mt-4 grid gap-3 sm:grid-cols-3">
            <button v-for="item in (['manual', 'automatic'] as AutomationMode[])" :key="item" class="rounded-lg px-4 py-3 text-sm font-semibold transition" :class="automationMode === item ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'" :disabled="busyAutomationMode || activeScene !== 'normal'" @click="switchAutomationMode(item)">{{ item === 'manual' ? '手动模式' : '自动模式' }}</button>
          </div>
          <p class="mt-4 text-xs text-slate-500">当前场景：{{ { normal: '正常', sleep: '睡眠', away: '离家' }[activeScene] }}。睡眠/离家场景锁定主控制模式，但自动安全联动、环境查询和门锁认证仍可用。</p>
          <div class="mt-5 border-t border-slate-100 pt-4">
            <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">场景预设</p>
            <div class="mt-3 grid gap-2 sm:grid-cols-3">
              <button v-for="item in (['normal', 'sleep', 'away'] as HomeScenario[])" :key="item" class="rounded-lg bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-50" :disabled="busyScene !== null" @click="runScene(item)">{{ { normal: '正常', sleep: '睡眠', away: '离家' }[item] }}</button>
            </div>
            <p class="mt-2 text-xs text-slate-500">正常开放全部执行器、阈值和主控制模式；睡眠开放 LED/风扇/蜂鸣器/门锁；离家开放水泵/蜂鸣器/门锁。门磁或设备异常时返回逐步失败结果。</p>
          </div>
        </article>
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <section class="grid gap-4 lg:grid-cols-[1fr_1fr]">
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            <div v-for="device in visibleDevices" :key="device.device_id" class="rounded-xl bg-slate-50 p-4">

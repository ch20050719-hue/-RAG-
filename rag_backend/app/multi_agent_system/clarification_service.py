...
class ClarificationService:
    ...
    INTENT_SUGGESTIONS: Dict[str, List[str]] = {
        ...
        "sensor_reading": ["读取温度湿度", "查看光照和人体状态", "查看书房环境"],
        "comfort_assessment": ["评估当前舒适度", "给出节能建议", "检查睡眠环境"],
        "scene_execution": ["执行睡眠模式", "执行离家模式", "执行节能模式"],
    }
    ...
    def _handle_low_confidence(self, query: str) -> ClarificationRequest:
        return ClarificationRequest(
            ...
            question="您是想查询设备、读取环境，还是执行家居场景？",
            suggestions=self._suggestions(query) or ["设备状态", "环境数据", "设备控制", "场景执行"],
    ...
    def _handle_ambiguous_intent(self, query: str) -> ClarificationRequest:
        return ClarificationRequest(
            ...
            question="请明确房间、设备和期望动作；如果是场景，请说明场景名称。",
            suggestions=self._suggestions(query) or ["查看设备状态", "读取环境数据", "执行睡眠模式", "执行离家模式"],
